...
import { buildOptimisticTeamMatch, buildTeamMatchFromReal, coerceManualPlayersToArray, getUpdatedMatchFields, mergePlayersUniqueByAccountId } from '@/hooks/team-operations-helpers';
...
function useTeamMatchOps(
  ...
  const addMatchToTeam = React.useCallback(async (matchId: number, teamSide: 'radiant' | 'dire') => {
    ...
    setTeams(prev => {
      ...
      if (team) {
        if (!(matchId in team.matches)) {
          const teamMatch: TeamMatchParticipation = (match && !match.isLoading && !match.error)
  ...
  const updateTeamMatchFromMatch = React.useCallback((matchId: number, match: ReturnType<MatchContextValue['getMatch']>) => {
...
function useTeamPlayerOps(
  ...
  const addPlayerToTeam = React.useCallback(async (playerId: number) => {
    if (!selectedTeamId) throw new Error('No active team selected');
    const teamKey = generateTeamKey(selectedTeamId.teamId, selectedTeamId.leagueId);
    ...
    setTeams(prev => {
      ...
      const team = newTeams.get(teamKey);
      if (team) {
        const existingManual = Array.isArray(team.manualPlayers) ? team.manualPlayers : Object.keys(team.manualPlayers || {}).map(n => Number(n));
  ...
  const removeManualPlayer = React.useCallback((playerId: number) => {
    if (!selectedTeamId) return;
    const teamKey = generateTeamKey(selectedTeamId.teamId, selectedTeamId.leagueId);
    setTeams(prev => {
      const newTeams = new Map(prev);
      const team = newTeams.get(teamKey);
      if (team) {
        const manual = coerceManualPlayersToArray(team.manualPlayers as number[] | Record<string, number | string> | undefined);
  ...
  const editManualPlayer = React.useCallback(async (oldPlayerId: number, newPlayerId: number) => {
    if (!selectedTeamId) return;
    const teamKey = generateTeamKey(selectedTeamId.teamId, selectedTeamId.leagueId);
    setTeams(prev => {
      const newTeams = new Map(prev);
