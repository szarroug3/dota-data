...
export function createErrorTeamData(teamId: number, leagueId: number, error: string): TeamData {
  return {
...
export function handleTeamDataErrors(
  teamId: number,
  leagueId: number,
  teamData: TeamResult,
...
export async function handleTeamSummaryOperation(
  teamId: number,
  leagueId: number,
  force: boolean,
  operationKey: string,
...
export function ensurePlaceholderTeam(
  setTeams: React.Dispatch<React.SetStateAction<Map<string, TeamData>>>,
  teamKey: string,
  teamId: number,
  leagueId: number,
