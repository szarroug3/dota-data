...
import { PlayerProvider, usePlayerContext } from '@/frontend/players/contexts/state/player-context';
...
jest.mock('@/hooks/use-player-operations', () => {
  const helpers = jest.requireActual('@/utils/player-helpers');
  return {
    usePlayerOperations: (state: any, _processing: any, playerDataFetching: any) => ({
      addPlayer: async (playerId: number) => {
        // Skip if already exists (mirror non-force behavior)
        if (state.players.has(playerId)) {
          return state.players.get(playerId) || null;
        }
        state.setIsLoading(true);
        const optimistic = helpers.createInitialPlayerData(playerId);
        state.setPlayers((prev: Map<number, any>) => {
          const m = new Map(prev);
          m.set(playerId, optimistic);
          return m;
        });
        try {
          await playerDataFetching.fetchPlayerData(playerId, false);
        } catch {
          // swallow to simulate graceful error handling
        } finally {
          state.setIsLoading(false);
        }
        return optimistic;
      },
      refreshPlayer: async (playerId: number) => {
        state.setIsLoading(true);
        try {
          await playerDataFetching.fetchPlayerData(playerId, true);
        } catch {
          // swallow to simulate graceful error handling
        } finally {
          state.setIsLoading(false);
