...
type CacheServiceWithQueue = { get: <T>(cacheKey: string, filename: string, ttl?: number) => Promise<T | null>; getQueueStats: () => unknown };
type CacheService = { get: (key: string) => Promise<unknown> };

// Refactor handlePollingRequest to extract queue/cached checks into helpers
function isSignatureActive(queueStats: unknown, service: string, signature: string): boolean {
  if (queueStats && typeof queueStats === 'object' && service in queueStats) {
    const serviceStats = (queueStats as Record<string, unknown>)[service];
    if (serviceStats && typeof serviceStats === 'object' && serviceStats !== null) {
      const s = serviceStats as { activeSignaturesList?: string[]; currentlyProcessing?: string };
      return Boolean(s.activeSignaturesList?.includes(signature) || s.currentlyProcessing === signature);
    }
  }
  return false;
}
...
export async function handlePollingRequest(
  ...
  cacheService: CacheServiceWithQueue,
  service: string
): Promise<Response | null> {
  if (!signature) return null;
  logWithTimestamp('log', `[handlePollingRequest] ===== POLLING REQUEST for signature: ${signature} =====`);
  const cachedData = await cacheService.get(signature, `${signature}.json`);
  if (cachedData) {
    logWithTimestamp('log', `[handlePollingRequest] Data ready in cache, returning result for signature: ${signature}`);
  ...
  if (isSignatureActive(queueStats, service, signature)) {
    logWithTimestamp('log', `[handlePollingRequest] Request still processing, returning 202 for signature: ${signature}`);
    return new Response(JSON.stringify({ status: 'processing', signature }), { status: 202, headers: { 'Content-Type': 'application/json' } });
  }
  logWithTimestamp('warn', `[handlePollingRequest] Request not found in cache or queue, may have failed for signature: ${signature}`);
  return new Response(JSON.stringify({ error: 'Request not found or may have failed' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
}

/**
 * Waits for a queued result to become available in the cache, polling until ready or timeout.
 * Used internally for async/queued processing.
 *
 * @template T
 * @param cacheService - The cache service instance.
 * @param signature - The signature/key to poll for.
 * @param options - Polling options (maxAttempts, pollInterval).
 * @returns Promise that resolves with the cached result or throws on timeout.
 */
export async function waitForQueuedResult<T = unknown>(
  ...
  logWithTimestamp('log', `[waitForQueuedResult] Starting to wait for signature: ${signature}`);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    logWithTimestamp('log', `[waitForQueuedResult] Checking cache attempt ${attempt + 1}/${maxAttempts} for signature: ${signature}`);
    
    // Try to get the result from cache
    const cachedResult = await cacheService.get(signature) as T;
    if (cachedResult) {
      logWithTimestamp('log', `[waitForQueuedResult] Found result in cache for signature: ${signature}`);
      return cachedResult;
    }
  }

  // If we get here, we've exceeded max attempts
  const timeoutError = new Error(`Timeout waiting for queued request to complete: ${signature}`);
  logWithTimestamp('error', `[waitForQueuedResult] ${timeoutError.message}`);
  throw timeoutError;
}
