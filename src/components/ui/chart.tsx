...
type ChartIcon = React.ComponentType<React.SVGProps<SVGSVGElement>>;
...
>(({ id, className, children, config, ...props }, ref) => {
  ...
  React.useEffect(() => {
    ...
    const updateSize = () => {
      ...
      setIsReady((prev) => (prev === nextReady ? prev : nextReady));
...
type ChartPayloadRecord = Record<string, string | number | undefined>;
type ChartPayloadData = Record<string, string | number | boolean | null | undefined> & {
  fill?: string;
  color?: string;
};
...
type ChartPayloadEntry = ChartTooltipPayload | LegendPayload;
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const payloadData = item.payload as ChartPayloadData | undefined;
            const indicatorColor = color || payloadData?.fill || payloadData?.color || item.color;
  ...
  return (
    ...
    >
      {payload.map((item) => {
        ...
        const rawKey = item.value ?? item.dataKey ?? 'legend-item';
        return (
          <div
            key={String(rawKey)}
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: ChartPayloadEntry, key: string) {
  ...
  const payloadRecord = payload as ChartPayloadRecord;
