        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Input
              ...
              onChange={(e) => onCustomDateRangeChange({ ...customDateRange, start: e.target.value || null })}
              className="text-xs"
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) => onCustomDateRangeChange({ ...customDateRange, end: e.target.value || null })}
              className="text-xs"
            />
          </div>
        </div>
...
function ResultFilter({
  ...
  onChange: (v: MatchFilters['result']) => void;
}) {
  return (
    <div>
...
function TeamSideFilter({
  value,
  onChange,
}: {
  value: MatchFilters['teamSide'];
  onChange: (v: MatchFilters['teamSide']) => void;
}) {
  return (
    <div>
...
function PickOrderFilter({
  value,
  onChange,
}: {
  value: MatchFilters['pickOrder'];
  onChange: (v: MatchFilters['pickOrder']) => void;
}) {
  return (
    <div>
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div>
              <Toggle pressed={value} onPressedChange={onChange} size="sm" className="px-3">
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ${className || ''}`}>
      ...
      <HighPerformersFilter
        ...
        onChange={(v) => handleChange('highPerformersOnly', v)}
      />
    </div>
  );
};
