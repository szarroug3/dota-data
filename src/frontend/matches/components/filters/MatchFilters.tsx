...
function DateRangeFilter({ value, onChange, customDateRange, onCustomDateRangeChange }: { value: MatchFilters['dateRange']; onChange: (v: MatchFilters['dateRange']) => void; customDateRange: MatchFilters['customDateRange']; onCustomDateRangeChange: (range: MatchFilters['customDateRange']) => void; }) {
  return (
      ...
      {value === 'custom' && (
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Input type="date" value={customDateRange.start || ''} onChange={(e) => onCustomDateRangeChange({ ...customDateRange, start: e.target.value || null })} className="text-xs" />
          </div>
          <div>
            <Label className="text-xs">End Date</Label>
            <Input type="date" value={customDateRange.end || ''} onChange={(e) => onCustomDateRangeChange({ ...customDateRange, end: e.target.value || null })} className="text-xs" />
          </div>
        </div>
      )}
    </div>
...
function HeroesPlayedFilter({ value, onChange, matches, teamMatches }: { value: string[]; onChange: (v: string[]) => void; matches: Match[]; teamMatches: Record<number, TeamMatchParticipation>; }) {
  ...
  matches.forEach(match => {
    ...
    if (match.draft) {
      ...
      picks?.forEach(pick => { if (pick.hero?.id) { heroIdSet.add(pick.hero.id); } });
    }
  });
  if (heroIdSet.size === 0) availableHeroes.forEach(hero => heroIdSet.add(hero.id));
  const playedHeroes = Array.from(heroIdSet).map(id => availableHeroes.find(h => h.id === id)).filter((h): h is Hero => !!h).sort((a, b) => a.localizedName.localeCompare(b.localizedName));
  const options = playedHeroes.map(hero => ({ value: hero.id, label: hero.localizedName }));
  return (
    <div>
      <Label className="mb-2 block">Heroes Played</Label>
      <MultiSelectCombobox options={options} value={value} onValueChange={onChange} placeholder="Search heroes..." searchPlaceholder="Search heroes..." emptyMessage="No heroes found." className="w-full" />
    </div>
  );
}
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div>
              <Toggle pressed={value} onPressedChange={onChange} size="sm" className="px-3">Matches with High Performing Heroes Only</Toggle>
            </div>
          </TooltipTrigger>
          <TooltipContent><p>5+ games with 60%+ win rate</p></TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </div>
  );
...
function OpponentFilter({ value, onChange, teamMatches }: { value: string[]; onChange: (v: string[]) => void; teamMatches: Record<number, TeamMatchParticipation>; }) {
  const opponentNames = useMemo(() => {
    const names = new Set<string>();
    Object.values(teamMatches).forEach(teamMatch => { if (teamMatch.opponentName) names.add(teamMatch.opponentName); });
  ...
  return (
    <div>
      ...
      <MultiSelectCombobox options={options} value={value} onValueChange={onChange} placeholder="Select opponents..." searchPlaceholder="Search opponents..." emptyMessage="No opponents found." className="w-full" />
    </div>
  );
}

export const MatchFilters: React.FC<MatchFiltersProps> = ({ filters, onFiltersChange, matches, teamMatches, className }) => {
  const handleChange = <K extends keyof MatchFilters>(key: K, value: MatchFilters[K]) => { onFiltersChange({ ...filters, [key]: value }); };
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ${className || ''}`}>
      <DateRangeFilter value={filters.dateRange} onChange={v => handleChange('dateRange', v)} customDateRange={filters.customDateRange} onCustomDateRangeChange={v => handleChange('customDateRange', v)} />
