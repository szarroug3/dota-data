...
import { ChartDataPoint, formatTime, formatYAxisTick, PerformanceTooltip as PartsPerformanceTooltip, renderEventDot } from './MatchDetailsPanelEventsParts';
...
function buildPerformanceData(goldData: GoldAdvantageData, experience: ExperienceAdvantageData): ChartDataPoint[] {
  return goldData.times.map((time: number, index: number) => {
    ...
    return { time, goldAdvantage: radiantGold - direGold, xpAdvantage: radiantXP - direXP, radiantGold, direGold, radiantXP, direXP, event: undefined };
  });
}
...
function filterImportantEvents(events: GameEvent[]): GameEvent[] {
  const isImportant = (event: GameEvent) => event.type === 'CHAT_MESSAGE_FIRSTBLOOD' || event.type === 'CHAT_MESSAGE_AEGIS' || event.type === 'team_fight';
...
function addEventsToTimeline(timeline: Map<number, ChartDataPoint>, events: GameEvent[], eventLineValue: number): void {
  events.forEach((event) => {
    ...
    timeline.set(event.time, { time: event.time, goldAdvantage: null, xpAdvantage: null, radiantGold: 0, direGold: 0, radiantXP: 0, direXP: 0, eventLine: eventLineValue, event });
  });
}
...
function prependFirstNegativeTickIfNeeded(data: ChartDataPoint[]): ChartDataPoint[] {
  ...
  return [{ time: firstNegativeTick, goldAdvantage: 0, xpAdvantage: 0, radiantGold: 0, direGold: 0, radiantXP: 0, direXP: 0, event: undefined }, ...data];
}

function createChartData(goldData: GoldAdvantageData, experienceAdvantage: ExperienceAdvantageData, events: GameEvent[]): ChartDataPoint[] {
...
const chartConfig = { goldAdvantage: { label: 'Gold Advantage', color: '#3b82f6' }, xpAdvantage: { label: 'XP Advantage', color: '#8b5cf6' } } as const;

interface PerformanceTimelineChartProps { chartData: ChartDataPoint[]; minTime: number; maxTime: number; chartMinAdvantage: number; chartMaxAdvantage: number; match?: Match; }
...
function PerformanceTimelineChart({ chartData, minTime, maxTime, chartMinAdvantage, chartMaxAdvantage, match }: PerformanceTimelineChartProps) {
  ...
  return (
    <ChartContainer config={chartConfig} className="h-[650px] w-full hidden @[350px]:block">
      <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
        ...
        <XAxis dataKey="time" tickFormatter={(value) => formatTime(value)} domain={[minTime - 90, maxTime]} ticks={generateTimeTicks(minTime - 90, maxTime)} allowDataOverflow={false} scale="time" />
        <YAxis tickFormatter={yTickFormatter} domain={[chartMinAdvantage - 1000, chartMaxAdvantage + 1000]} tickCount={10} />
        <ReferenceLine y={0} stroke="#666" strokeDasharray="3 3" />
        <ChartTooltip content={<PartsPerformanceTooltip match={match} />} />
        <Line key="gold-advantage" type="monotone" dataKey="goldAdvantage" stroke="var(--color-goldAdvantage)" strokeWidth={2} dot={false} connectNulls name="Gold Advantage" />
        <Line key="xp-advantage" type="monotone" dataKey="xpAdvantage" stroke="var(--color-xpAdvantage)" strokeWidth={2} strokeDasharray="5 5" dot={false} connectNulls name="XP Advantage" />
        <Line key="events" type="monotone" dataKey="eventLine" stroke="transparent" strokeWidth={0} dot={renderEventDot} name="Events" />
...
function PerformanceChart({ match }: { match?: Match }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between"><h3 className="text-lg font-semibold truncate">Performance Timeline</h3></div>
      <div className="text-xs text-muted-foreground flex"><span className="truncate flex-1">Advantage: Positive values (top) show Radiant advantage, negative values (bottom) show Dire advantage</span></div>
      <div className="relative">
        <PerformanceTimelineChart chartData={chartData} minTime={minTime} maxTime={maxTime} chartMinAdvantage={chartMinAdvantage} chartMaxAdvantage={chartMaxAdvantage} match={match} />
      </div>
    </div>
  );
}
