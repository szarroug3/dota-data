...
export const PlayerDetailsPanel: React.FC<PlayerDetailsPanelProps> = React.memo(({ player, viewMode, onViewModeChange, allPlayers = [], hiddenPlayerIds = new Set<number>(), heroes, matchesArray, selectedTeam }) => {
  return (
      ...
      <CardContent className="flex-1 overflow-y-auto min-h-0 @[90px]:block hidden">
        {viewMode === 'summary' && (
          <div className="space-y-4">
            <PlayerDetailsPanelSummary player={player} allPlayers={allPlayers} hiddenPlayerIds={hiddenPlayerIds} heroes={heroes} />
          </div>
        )}
        {viewMode === 'details' && (
          <div className="space-y-4">
            <PlayerDetailsPanelDetails player={player} _allPlayers={allPlayers} _hiddenPlayerIds={hiddenPlayerIds} heroes={heroes} />
          </div>
        )}
        {viewMode === 'team' && (
          <div className="space-y-4">
            <PlayerDetailsPanelTeam player={player} _allPlayers={allPlayers} _hiddenPlayerIds={hiddenPlayerIds} heroes={heroes} matchesArray={matchesArray} selectedTeam={selectedTeam} />
          </div>
        )}
      </CardContent>
    </Card>
